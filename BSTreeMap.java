...
public class BSTreeMap<K extends Comparable<K>, V> implements MyMap<K, V> {
    ...
    protected Node<K, V> createBST(Pair<K, V>[] pairs, int low, int high) {
        ...
        int mid = low + (high-low) / 2; //find the mid index to split the array segment

        Node<K, V> node = new Node<>(pairs[mid].key, pairs[mid].value); //create a node for the middle element
        size++;
        node.setLeft(createBST(pairs, low, mid - 1)); //recursively create left subtree from left part of array

        if(node.getLeft() != null){
            node.getLeft().setParent(node);
        }

        node.setRight(createBST(pairs, mid + 1, high));
    ...
    private int preorder(Node<K, V> n, StringBuilder builder,
        ...
        if (nodesVisited > 0){    //check with TA
            builder.append(", ");
        }
    ...
    private int postorder(Node<K, V> n, StringBuilder builder,
       ...
       nodesVisited = postorder(n.getRight(), builder, nodesVisited);
        ...
        if (nodesVisited > 0){
            builder.append(", "); //ask TA if anything else need to do
        }
        builder.append(n.toString());
        return ++nodesVisited;
}
    ...
    private Node<K, V> iterativeSearch(K key) {
        Node<K, V> node = root;
        while(node != null) { //is second part necessary
            int comp = key.compareTo(node.key);
            if (comp < 0) {
                node = node.getLeft(); //move to left if key is smaller
            } else if (comp > 0) {
                node = node.getRight();
            } else {
                return node; //key found, return current node
            }
        }
            return null;
        // TODO
    ...
    public V put(K key, V value) {
        Node<K, V> z = new Node<>(key, value); //node to insert
        Node<K, V> y = null; // parent of new node to insert
        Node<K, V> x = root;

        while(x != null){
            y = x;
            if(z.key.compareTo(x.key) < 0){
                x = x.getLeft(); //if new node value is less than current node value
        ...
        z.setParent(y); //set parent of new node z to y

        //i tree is empty set new node as root
        if (y == null) {
            root = z;
        } else if (z.key.compareTo(y.key) < 0){
            y.setLeft(z); //insert z as left child if its key is less than y's key
        } else{
            y.setRight(z);
        }
        size++;
    ...
    private int sumLevels(Node<K, V> node, int level) {
        ...
        int tot = level;
        return tot + sumLevels(node.getLeft(), level+1) + sumLevels(node.getRight(), level + 1);
